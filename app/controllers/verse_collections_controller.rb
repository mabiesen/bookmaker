class VerseCollectionsController < ApplicationController
  ...
  def create
    ...
    verses.each do |verse|
      @verse_collection.bible_verses << verse unless @verse_collection.bible_verse_ids.include? verse.id
    end

    respond_to do |format|
      if @verse_collection.save
        format.html do
          redirect_to verse_collection_url(@verse_collection), notice: 'Verse collection was successfully created.'
        end
  ...
  def update
    respond_to do |format|
      if @verse_collection.update(verse_collection_params)
        format.html do
          redirect_to verse_collection_url(@verse_collection), notice: 'Verse collection was successfully updated.'
        end
